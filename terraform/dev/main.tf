...
resource "aws_iam_role" "ecs_task_execution" {
  name = "ecsTaskExecutionRole-hackathon-dev"
...
resource "aws_cloudwatch_log_group" "order" {
  name = "/ecs/order-service-dev"
}

resource "aws_cloudwatch_log_group" "storage" {
  name = "/ecs/storage-service-dev"
}
...
resource "aws_acm_certificate" "cert" {
  ...
  provisioner "local-exec" {
    when    = "create"
    command = "echo ACM certificate requested for ${var.domain_name}"
  }
}
...
# Create Route53 record pointing domain root to ALB
...
resource "aws_security_group_rule" "allow_alb_to_ecs_3000_graf" {
  type                     = "ingress"
  from_port                = 3000
  to_port                  = 3000
  protocol                 = "tcp"
